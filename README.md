...
2. Games missing any of **published year / manufacturer / genre** are sent
   ...
   — the AI looks each one up online (one search per game) and verifies
   year/publisher/genre before answering (≈$0.01 per search + tokens,
   shown in the log; opt out via the *Web-search games the AI can't
   identify* checkbox).

   **Cost control** — every answer is remembered in
   `data\enrich_cache.json`: identified games are reused **free**
   across runs *and* across systems (duplicate entries in e.g. a
   Favorites wheel cost nothing); clones sharing a description inside
   a system are answered **once**; games that stayed unknown even
   after web search are **skipped on later runs** instead of being
   re-billed (tick *Retry games that previously failed* to search
   them again). The log reports cache hits, clone savings, and
   skipped-failed counts per system.
...
`default.zip`, any folders inside the art-type folders, and — in the
**parent MAME system's folder only** (the one subset wheels fall back
to) — any video another system lists without holding its own copy.
Subset wheels' own folders are read by nobody else and get normal
per-system cleanup; matching ignores caps, and a *"folder not found"*
note in the analysis line flags art-path mismatches. Removed
...
1. **Convert 16:9** — converts the system's 4:3 themes to widescreen.
   ...
   artwork** (compiled SWF — the converter cannot resize it) convert
   normally, and the **recorder** re-registers the bezel (suite v2.9):
   the flash layer is squeezed ×0.75 about the video center at render
   time, so the recorded theme shows the game at true 4:3 with the
   bezel hugging it and no flash art stretched (the dmnfrnt case).
   XML-drawn `bsize`/`bcolor` borders always follow the video and are
   unaffected.
